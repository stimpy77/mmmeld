...
def calculate_total_duration(main_audio_path, image_inputs, start_margin, end_margin):
    ...
    if main_audio_path:
        ...
        return max(main_audio_duration + start_margin + end_margin, 5)  # Ensure minimum 5 seconds
    else:
        total_duration = 0
        ...
        return max(total_duration, 5)  # Ensure minimum 5 seconds
...
def create_visual_sequence(image_inputs, total_duration, temp_folder):
    ...
    for i, input_path in enumerate(image_inputs):
        ...
        if is_video(input_path):
            filter_complex.append(f"[{i}:v]fps=30,format=yuv420p[v{i}];")
        else:
            filter_complex.append(f"[{i}:v]loop=duration=5:size=1,fps=30,format=yuv420p[v{i}];")
    ...
    cmd = ["ffmpeg", "-y"] + inputs + ["-filter_complex", "".join(filter_complex),
           "-map", "[outv]", "-c:v", "libx264", "-preset", "ultrafast", "-crf", "0", str(temp_sequence)]
...
def generate_video(image_inputs, audio_path, bg_music_path, output_path, bg_music_volume, start_margin, end_margin, temp_folder):
    ...
    visual_sequence = create_visual_sequence(processed_inputs, total_duration, temp_folder)
    ...
    if bg_music_path:
        ...
        filter_complex.append(f"[{bg_index}:a]aloop=-1:size=44100,volume={bg_music_volume}[bg_music];")
    
    filter_complex.append(f"[0:v]fps=30,format=yuv420p,loop=loop=-1:size=1[looped_video];")
    filter_complex.append(f"[looped_video]trim=duration={total_duration},setpts=PTS-STARTPTS[final_video];")
    ...
    fade_duration = min(2, total_duration / 2)  # Ensure fade duration is not longer than half the total duration
    filter_complex.append(f"[final_video]fade=t=out:st={max(0, total_duration-fade_duration)}:d={fade_duration}[faded_video];")
